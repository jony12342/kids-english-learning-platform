...
**⚠️ 开发策略：先用免费方案，后期按需升级**

#### MVP阶段：完全免费（推荐）

| 服务 | 用途 | 选择理由 | 成本 |
|------|------|----------|------|
| **Web Speech API** | 语音识别 + 语音合成 | - 浏览器原生，完全免费<br>- 无需API密钥<br>- 隐私保护好<br>- 快速开发 | **¥0** |
...
- ⚠️ 准确率比专业服务低（但够用）
- ⚠️ 声音为默认成人声音（可接受）
- ⚠️ 浏览器兼容性（Safari/Chrome最好）

#### 正式上线后：混合方案（可选升级）

如果后期需要更好的准确率和儿童声音，可升级到混合方案：

| 服务 | 用途 | 比例 | 成本 |
|------|------|------|------|
| **Web Speech API** | 规则对话（简单） | 70% | ¥0 |
| **科大讯飞** | 复杂AI对话 | 30% | ¥76.5/月 |

**升级后成本**：
- 100活跃用户 × 10次对话/天 × 30天 = 30,000次
- 70%免费（21,000次）× ¥0 = ¥0
- 30%付费（9,000次）× ¥0.0085/次 = ¥76.5
- **总计：¥76.5/月**
